...
class WithdrawsController extends VoyagerBaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        //Custom Code: To check if Withdraw is already successful before changing the status
        if ($request->status == 1){
            $statusCheck = Userwithdraw::where('id', $id)->first();

            if($statusCheck->status == 1){
                return back()->with([
                    'message'    => __('Already Successful! Error Updating')." {$dataType->getTranslatedAttribute('display_name_singular')}",
                    'alert-type' => 'error',
                ]);;
            }

            if($statusCheck->method == "Paystack")
            {
                $fields = [
                ...
                $responds = finalizeTransfer($fields);

                //dd($responds);
            }

            $userWithdrawCase = UserProfile::with('user')->where('user_id', $request->user_id)->first();
            $withdraw = Userwithdraw::whereReference($request->reference)->first();

            Notification::send($userWithdrawCase->user, new withdrawalSuccessful($withdraw));
            $userWithdrawCase->user->userprofile->main_balance = $userWithdrawCase->user->userprofile->main_balance - $request->amount;
            $userWithdrawCase->user->userprofile->save();
        }
